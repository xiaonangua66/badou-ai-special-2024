...
def cannyDetail():
    ...
    sigma = 0.5
    dim = 5
    gas_filter = np.zeros([dim, dim])
    tmp = [i - dim // 2 for i in range(dim)]
    n1 = 1 / (2 * math.pi * sigma ** 2)
    n2 = -1 / (2 * sigma ** 2)
    for i in range(dim):
        for j in range(dim):
            gas_filter[i, j] = n1 * math.exp(n2 * (tmp[i] ** 2 + tmp[j] ** 2))
    gas_filter = gas_filter / gas_filter.sum()
    dx, dy = img.shape
    ...
    for i in range(dx):
        for j in range(dy):
            img_new[i, j] = np.sum(img_pad[i:i + dim, j:j + dim] * gas_filter)
    plt.figure(1)
    ...
    high_boundary = lower_boundary * 3  # 高阈值为低阈值的三倍
    zhan = []
